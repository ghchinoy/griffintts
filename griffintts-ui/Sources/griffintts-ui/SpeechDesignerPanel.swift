...
struct SpeechDesignerPanel: View {
    ...
    var body: some View {
        // VStack fills the full sidebar height. TextEditor takes remaining space
        // after the fixed-height controls at the bottom.
        VStack(alignment: .leading, spacing: 0) {

            // ── Prompt (grows with window height) ─────────────────────
            VStack(alignment: .leading, spacing: 6) {
                ...
                TextEditor(text: $prompt)
                    ...
                    .cornerRadius(8)
                    .frame(maxHeight: .infinity) // fills all available space
                    .overlay(
