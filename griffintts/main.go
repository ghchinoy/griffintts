...
type SpeakRequest struct {
	...
	DurationStretch float64 `json:"duration_stretch,omitempty"`
}
...
func executeSynthesis(args []string) {
	...
	if isNative {
		...
		if speed != 1.0 {
			synthArgs = append(synthArgs, "-r", fmt.Sprintf("%.2f", speed))
		}
		synthArgs = append(synthArgs, "-ow", outWav, tempLabPath)

		synthCmd := exec.Command(enginePath, synthArgs...)
