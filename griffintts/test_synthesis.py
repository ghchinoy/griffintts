...
def generate_full_context_labels(phones, phone_features):
    ...
    # Sensible defaults for all 113 keys from en_us_world.contexts
    ...
    for i, center_phone in enumerate(phones):
        ...
        # Let's start with a pipe, append key:val| for all keys, and ensure it matches Jibo's trees
        parts = []
        for k, v in lbl.items():
            parts.append(f"{k}:{v}")
        label_string = "|".join(parts)
        # Jibo's trees check for "*|KEY:val|*" or similar, so let's wrap it nicely
...
if __name__ == "__main__":
    phones_path = "tools/griffintts/assets/en_us_world/en_us_world.phones"
    voice_path = "tools/griffintts/assets/en_us_world/en_us_world.voice"
    ...
    lab_file_path = "tools/griffintts/hello.lab"
    ...
    # Run hts_engine
    wav_file_path = "tools/griffintts/hello.wav"
    ...
    try:
        ...
        print("HTS Engine execution completed successfully!")
